...
var conf = module.exports = convict({
  ...
  public_url: {
    ...
    format: 'string = "https://persona.org"',
    env: 'URL'
  },
  scheme: {
 ...
 * if the host, port, or scheme are different than https://persona.org:443
...
module.exports.performSubstitution = function(app) {
  if (conf.get('public_url') != 'https://persona.org') {
    app.use(postprocess(function(req, buffer) {
      return buffer.toString().replace(new RegExp('https://persona.org', 'g'), conf.get('public_url'));
    }));
  }
};
