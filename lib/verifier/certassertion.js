...
function compareAudiences(want, got) {
  ...
  try {
    ...
    // 2. domain uses a special protocol: resource, app, chrome-extension
    // 3. domain and port 'rp.tld:8080'
    // 4. domain only 'rp.tld'
    ...
    // case 2
    else if (/^(?:resource|app|chrome-extension):\/\//.test(got)){
      var gu = normalizeParsedURL(url.parse(got));
      got_scheme = gu.protocol;
      got_domain = gu.hostname;
    }
  ...
  jwcrypto.cert.verifyBundle(
    ...
    new Date(), function(issuer, next) {
      ...
      if (issuer === HOSTNAME) return next(null, publicKey);
        ...
        if (config.get('forcable_issuers').indexOf(forceIssuer) === -1) {
          return errorCB("this verifier doesn't won't force issuer for " + forceIssuer);
        } else {
