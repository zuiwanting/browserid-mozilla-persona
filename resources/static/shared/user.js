...
BrowserID.User = (function() {
  ...
  User = {
     ...
     * cert/keypair.  Note, we do not authenticate to persona.org but
     * merely get an assertion for persona.org so that we can either add the
        ...
        if(authInfo.authenticated) {
          persistEmailKeypair(email, "primary", authInfo.keypair, authInfo.cert,
            function() {
              ...
              User.getAssertion(email, "https://persona.org", function(assertion) {
                if (assertion) {
                  onComplete("primary.verified", {
                    assertion: assertion
                  });
    ...
    /**
     * Sync local identities with persona.org.  Generally should not need to
     * be called.
     * @method syncEmails
     * @param {function} [onComplete] - Called whenever complete.
     * @param {function} [onFailure] - Called on error.
  ...
  // the origin of the of the RP.  On persona.org, it will remain the origin of
  // persona.org
